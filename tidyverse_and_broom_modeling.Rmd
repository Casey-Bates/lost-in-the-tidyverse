...
## Compute residuals for all dataframes 
...
home_sales %>% # Original dataset with errors for Rifle!
  nest(-location) %>% 
  mutate(models = map(data, ~lm(sale_price ~ square_feet, .))) %>%
  mutate(resids = map2(data, models, add_residuals)) %>%
  unnest(resids) %>% 
